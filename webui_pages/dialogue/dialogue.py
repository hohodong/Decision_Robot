...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    if not chat_box.chat_inited:
        st.toast(
            f"欢迎使用 [`应急管理问答系统`](https://github.com/hohodong/Decision_Robot) ! \n\n"
    ...
    with st.sidebar:
        ...
        # running_models = ["/root/models/internlm2_5-1_8b-chat"]
        available_models = []
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            ...
            if dialogue_mode == "LLM 对话":
            ...
            elif dialogue_mode == "应急管理问答":
                from webui_pages.dialogue.multi_agents import MultiAgent
                multiagent = MultiAgent(chat_box, api)
                ...
                text = ""
                text += multiagent.multi_agent_answer(query=prompt, selected_kb=selected_kb,
                                                 kb_top_k=kb_top_k,
